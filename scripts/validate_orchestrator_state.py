#!/usr/bin/env python3
"""Validate MAT-4 orchestrator state JSON Schema and example documents (requires requirements-dev.txt)."""
...
_BUNDLE_KEY = "orchestrator-state@v1"
_ENV_BUNDLE_DIR = "MAT_ORCHESTRATOR_STATE_V1"
...
def resolve_bundle_dir(root: Path) -> Path:
    ...
    return (root / "schemas" / "orchestrator-state" / "v1").resolve()
...
def _build_registry(schema: dict) -> object:
    ...
    uri = (V1 / "orchestrator-state.schema.json").as_uri()
...
def main() -> None:
    ...
    if schema.get("title") is None:
        raise SystemExit("orchestrator-state.schema.json should include a title for humans")
    ...
    for path in sorted(valid_dir.glob("*.json")):
        ...
        print(f"OK state    {path.relative_to(ROOT)}")
    ...
    print(f"All MAT-4 schema checks passed (bundle: {V1}).")
