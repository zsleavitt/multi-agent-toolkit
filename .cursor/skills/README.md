...
To use MAT skills while Cursor has **another repo** open as the workspace root, install **personal** copies generated by setup:

```bash
bin/setup       # macOS/Linux — also writes ~/.cursor/skills/multi-agent-toolkit-*
```

```powershell
.\bin\setup.ps1 # Windows — writes %USERPROFILE%\.cursor\skills\multi-agent-toolkit-*
```

Implementation: [`scripts/install_cursor_personal_skills.py`](../../scripts/install_cursor_personal_skills.py) reads these stubs and rewrites paths plus prefixed slash names (`/multi-agent-toolkit-develop`, …). See [`adapters/cursor/README.md`](../../adapters/cursor/README.md).

Fully restart Cursor after installing or upgrading.
